...
!!! note "یادداشت"
	...
	درباره عملگرهایی مانند [`(+)`][addition]، [`(/)`][division] و [`(++)`][append]در مستندات ماژول [`Basics`][basics] می‌توانید اطلاعات بیشتری کسب کنید. در یک مقطع، خواندن تمام مستندات آن بسته ارزشمند است!
...
!!! note "یادداشت"
	...
	افرادی که از زبان‌هایی مانند جاوااسکریپت می‌آیند ممکن است تعجب کنند که چرا توابع در اینجا متفاوت به نظر می‌رسند:
...
این می‌تواند بسیار مفید باشد، اما وقتی اوضاع پیچیده‌تر می‌شود، معمولا بهتر است به جای تاپِل از رکورد استفاده کنید.

## رکورد {#record}

یک **رکورد** می‌تواند مقادیر زیادی را نگه دارد و هر مقدار با یک نام مرتبط است.

در ادامه، یک رکورد وجود دارد که اقتصاددان بریتانیایی جان آ. هابسون را ذخیره می‌کند:
...
یک رکورد با سه **فیلد** تعریف کردیم که اطلاعاتی درباره نام، نام خانوادگی و سن جان را شامل می‌شود.
