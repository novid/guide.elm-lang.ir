    ...
    Success quote ->
      div []
        ...
        , p [ style "text-align" "right" ]
            ...
            , text (" توسط " ++ quote.author ++ " (" ++ String.fromInt quote.year ++ ")")
            ]
        ]
...
این برنامه به طور کلی مشابه مثال قبلی است:

- تابع `init` برنامه را در حالت `Loading` شروع می‌کند، با یک دستور برای دریافت یک نقل قول تصادفی.
...
تفاوت اصلی در تعریف `getRandomQuote` است. به جای استفاده از `Http.expectString` به `Http.expectJson` تغییر کرده‌ایم. این چه معنایی دارد؟

## JSON

زمانی که از [`/api/random-quotes`](https://elm-lang.org/api/random-quotes) برای یک نقل قول تصادفی درخواست می‌کنید، سرور یک رشته JSON مانند این تولید می‌کند:
...
بسته [`elm/json`][json] ماژول [`Json.Decode`][decode] را در اختیار ما می‌گذارد. این ماژول پر از دیکودرهای کوچک است که می‌توانیم آن‌ها را به هم متصل کنیم.

[json]: https://package.elm-lang.org/packages/elm/json/latest/
[decode]: https://package.elm-lang.org/packages/elm/json/latest/Json-Decode

بنابراین برای دریافت `"age"` از `{ "name": "تام", "age": 42 }` دیکودر زیر را ایجاد می‌کنیم:
...
تابع [`field`][field] دو آرگومان می‌گیرد:
...
بنابراین `field "age" int` درخواست یک فیلد `"age"` را می‌کند و اگر وجود داشته باشد، دیکودر `Int` را اجرا می‌کند تا سعی کند یک عدد صحیح استخراج کند.
...
اما اگر بخواهیم دو فیلد را دیکود کنیم چه؟ دیکودرها را با استفاده از تابع [`map2`](https://package.elm-lang.org/packages/elm/json/latest/Json-Decode#map2) به هم متصل می‌کنیم:
...
بنابراین اگر از `personDecoder` روی `{ "name": "تام", "age": 42 }` استفاده کنیم، یک مقدار Elm مانند `Person "تام" 42` دریافت خواهیم کرد.
...
تابع [`map2`](https://package.elm-lang.org/packages/elm/json/latest/Json-Decode#map2) و [`map4`](https://package.elm-lang.org/packages/elm/json/latest/Json-Decode#map4) را برای مدیریت اشیا با فیلدهای متعدد دیدیم. اما وقتی شروع به کار با اشیای بزرگ‌تر JSON می‌کنید، پیشنهاد می‌کنم نگاهی به بسته [`NoRedInk/elm-json-decode-pipeline`](https://package.elm-lang.org/packages/NoRedInk/elm-json-decode-pipeline/latest) بیندازید. این بسته، نوع داده پیچیده‌تری دارد، اما برخی توسعه‌دهندگان کار با آن را ترجیح می‌دهند.

> **واقعیت جالب:** داستان‌های زیادی شنیده‌ام که هنگام تغییر از JS به Elm، باگ‌هایی در کد سرور پیدا شده است. دیکودرهایی که توسعه‌دهندگان می‌نویسند به عنوان یک مرحله اعتبارسنجی عمل کرده و موارد عجیب در مقادیر JSON را شناسایی می‌کنند. بنابراین وقتی NoRedInk از React به Elm تغییر کرد، چند باگ در کد Ruby آن‌ها را آشکار کرد!