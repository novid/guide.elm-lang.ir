...
تا به حال روی دستورات تمرکز داشتیم. با نمونه‌های HTTP و مقدار تصادفی، به Elm دستور دادیم که کار خاصی را بلافاصله انجام دهد، اما این الگوی عجیبی برای یک ساعت است. ما _همیشه_ می‌خواهیم زمان فعلی را بدانیم. اینجاست که مفهوم **اشتراک یا subscription** وارد می‌شود!

با کلیک روی دکمه "ویرایش" شروع و کد برنامه را مطالعه کنید.
...
مفاهیم جدید همه از بسته [`elm/time`][elm-time] می‌آیند. بیایید این بخش‌ها را بررسی کنیم!
...
خوب، حالا چگونه باید `Time.Posix` را دریافت کنیم؟ با یک **اشتراک یا subscription**!
...
ما از تابع [`Time.every`][time.every] استفاده می‌کنیم:
...
1. یک بازه زمانی به میلی‌ثانیه. `1000` میلی‌ثانیه که به معنای هر ثانیه است. اما می‌توانیم از `60 * 1000` برای هر دقیقه یا `5 * 60 * 1000` برای هر پنج دقیقه استفاده کنیم.
