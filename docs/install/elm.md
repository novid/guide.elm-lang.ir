...
با اجرای این دستور، یک سرور در نشانی [`http://localhost:8000`][localhost] راه‌اندازی می‌شود. می‌توانید هر فایل Elm را اجرا کنید تا ببینید چگونه کامپایل می‌شود. دستور `elm reactor` را اجرا کنید، به لینک localhost:8000 بروید و سعی کنید فایل `src/Main.elm` را بررسی کنید!
...
!!! abstract "یادداشت مترجم"
	...
	`elm-json`
	: مدیریت بسته‌ها و وابستگی‌های نرم‌افزاری
