...
!!! note "یادداشت"
	...
	این داده اضافی "Flag" نامیده می‌شود زیرا شبیه پرچم‌های خط فرمان است. شما می‌توانید دستور `elm make src/Main.elm` را با برخی پرچم‌ها مانند `optimize--` و `output=main.js--` فراخوانی کرده تا عملکرد اولیه آن را تغییر دهید.
...
Elm با بررسی نوع داده ورودی، اطمینان می‌یابد که پرچم دقیقا همان چیزی است که انتظار دارید. بدون این بررسی، می‌توانید هر چیزی را ارسال کنید که منجر به بروز خطای زمان اجرا در Elm می‌شود!
...
قبل از اینکه راهی برای دیکودِرهای JSON پیدا شود، سایر انواع داده وجود داشته‌اند. اگر تصمیم به استفاده از آن‌ها گرفتید، برخی نکات ظریف وجود دارد که باید به آن‌ها توجه کنید. در هر یک از نمونه‌های زیر، نوع داده پرچم مورد نظر و در زیر آن مقادیر مختلف جاوااسکریپت آمده است:
